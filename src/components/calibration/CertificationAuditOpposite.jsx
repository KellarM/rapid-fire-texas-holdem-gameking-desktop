...
import { runBetAuditWithAbort } from '@/lib/workerBridgeOpposite';
...
function getStorageKeys(moduleId) {
  return {
    results: `certAuditOpp_${moduleId}_results`,
    progress: `certAuditOpp_${moduleId}_progress`,
    checkpoint: `certAuditOpp_${moduleId}_checkpoint`,
  };
}
...
// ── Database persistence helpers — reuses CertAuditResult entity with '_opp' module ids ──
...
export default function CertificationAuditOpposite() {
  ...
  const exportPDF = () => {
    ...
    doc.text('Rapid Fire Texas 10 (Opposite Deck) — Certification Audit Report', 10, 10);
    doc.setFontSize(8);
    doc.setTextColor(200, 200, 200);
    doc.text('GLI-11 / BMM / eCOGRA Standards Compliance', 10, 17);
    doc.setFontSize(7);
    doc.text(`Generated: ${now}  |  Opposite 32-card engine  |  Multi-tier certification`, pageW - 10, 17, { align: 'right' });
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(
        `Rapid Fire Texas 10 (Opposite Deck) — Certification Audit  |  GLI-11 / BMM / eCOGRA  |  Page ${i} of ${totalPages}`,
        pageW / 2,
        doc.internal.pageSize.getHeight() - 5,
        { align: 'center' }
      );
    }
    doc.save(`RapidFire_Opposite_CertAudit_${new Date().toISOString().slice(0, 10)}.pdf`);
  };
  ...
  const exportWord = () => {
    ...
    const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">
      <head><meta charset="utf-8"><title>Certification Audit Report — Opposite Deck</title></head>
      <body style="font-family:Arial,sans-serif;font-size:9pt;">
        <h2>Rapid Fire Texas 10 (Opposite Deck) &mdash; Certification Audit Report</h2>
        <p style="color:#444;">Generated: ${now} | Standards: GLI-11 / BMM Technical / eCOGRA | Opposite 32-card engine</p>
        <table style="border-collapse:collapse;width:100%;font-size:8.5pt;">${tableRows}</table>
        <p style="color:#888;font-size:8pt;margin-top:20px;">This report was generated by the Rapid Fire Texas 10 Gaming License Calibration 2 (Opposite Deck) Tool. All simulations run entirely in-browser using a certified 32-card engine.</p>
      </body></html>`;
    ...
    a.href = url;
    a.download = `RapidFire_Opposite_CertAudit_${new Date().toISOString().slice(0, 10)}.doc`;
    a.click();
  ...
  const exportModuleCertificate = (moduleId) => {
    ...
    const certNo = `RF-OPP-${module.id.toUpperCase()}-${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}`;
    ...
    doc.setFontSize(9.5);
    doc.setFont('helvetica', 'bold');
    doc.text("RAPID FIRE TEXAS HOLD'EM — OPPOSITE DECK", pW / 2, 17, { align: 'center' });
    doc.setFontSize(6.5);
    ...
    doc.setFont('helvetica', 'normal');
    [
      `This certificate confirms that the above-named opposite-deck game engine has undergone a rigorous Monte Carlo statistical audit under the ${module.standard} standard.`,
    ...
    [
      ...
      { label: 'Engine:',           value: 'Rapid Fire Texas 10 — Opposite Deck In-Browser Monte Carlo v1.0' },
    ].forEach((f, i) => {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
      `Rapid Fire Texas 10 (Opposite Deck)  ·  ${module.name} Certification  ·  ${module.standard}  ·  ${dateStr}  ·  Page 1 of 1`,
    ...
    const startDataPage = () => {
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text('RAPID FIRE TEXAS HOLD\'EM — OPPOSITE DECK DETAILED RESULTS', dpW / 2, 18, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
      `Rapid Fire Texas 10 (Opposite Deck)  ·  ${module.name} Certification  ·  ${module.standard}  ·  ${dateStr}  ·  Page 1 of ${totalPages}`,
      pW / 2, 200, { align: 'center' }
    );
    for (let i = 2; i <= totalPages; i++) {
      ...
      doc.text(
        `Rapid Fire Texas 10 (Opposite Deck)  ·  ${module.name} Certification  ·  ${module.standard}  ·  ${dateStr}  ·  Page ${i} of ${totalPages}`,
        dpW / 2, dpH - 12, { align: 'center' }
      );
    }

    doc.save(`RapidFire_Opposite_Certificate_${module.id}_${now.toISOString().slice(0, 10)}.pdf`);
  };
      ...
      <div className="bg-slate-800/40 border border-slate-700 rounded-xl p-4 mb-2">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-white mb-1">Multi-Tier Certification Audit — Opposite Deck</h3>
              <p className="text-gray-400 text-sm">
                Four escalating audit modules covering all {ALL_BETS.length} betting positions (10 opposite hands + {PER_HAND_RANK_BETS.length} per-hand ranks + 8 color/river) with live payouts from{' '}
